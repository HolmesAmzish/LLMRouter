...
import cn.arorms.llm.router.app.entity.ModelCatalogEntity
import cn.arorms.llm.router.app.repository.ModelCatalogRepository
import cn.arorms.llm.router.app.repository.ProviderAccountRepository
import cn.arorms.llm.router.common.ModelListResponse
import cn.arorms.llm.router.common.ModelResponse
...
class ModelService(
    ...
    fun localModels(): ModelListResponse = ModelListResponse(
        provider = "llm-router",
        data = catalogRepository.findByEnabledTrue().map { ModelResponse(it.modelId, it.ownedBy ?: "llm-router", it.enabled) }
    ...
    suspend fun sync(accountId: Long): ModelListResponse = withContext(Dispatchers.IO) {
        ...
        catalogRepository.saveAll(models.map { ModelCatalogEntity(account.id, it.id, it.ownedBy, true) })
    ...
    private suspend fun fetchModels(accountId: Long, protocol: Protocol, baseUrl: String, apiKey: String): List<ModelResponse> {
        ...
        return when (protocol) {
            Protocol.OPENAI, Protocol.ANTHROPIC -> body.path("data").map { ModelResponse(it.path("id").asText(), it.path("owned_by").asText(it.path("ownedBy").asText(protocol.name.lowercase()))) }
            Protocol.GEMINI -> body.path("models").map { ModelResponse(it.path("name").asText().removePrefix("models/"), "google", it.path("supportedGenerationMethods").any { method -> method.asText() == "generateContent" }) }
        }
    }
}
