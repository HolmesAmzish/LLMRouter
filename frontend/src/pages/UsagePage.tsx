...
import type { UsagePageResponse, UsageRecord } from '../types'
import { Alert, Button, Card, CardHeader, EmptyState, Field, inputClass } from '../components/ui'

const emptyFilter = { from: '', to: '', provider: '', accountName: '', model: '', sessionId: '', apiKeyId: '', requestId: '' }

function freshInputTokens(record: UsageRecord): number {
  if (record.inputTokenSemantics === 'TOTAL') return Math.max(0, record.inputTokens - record.cacheReadTokens)
  ...
  return (
    <Card>
      ...
      <div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-8">
        ...
        <Field label="Account"><input className={inputClass} value={filter.accountName} onChange={(event) => setFilter({ ...filter, accountName: event.target.value })} /></Field>
        ...
        <Field label="API key ID"><input className={inputClass} type="number" value={filter.apiKeyId} onChange={(event) => setFilter({ ...filter, apiKeyId: event.target.value })} /></Field>
        <Field label="Request ID"><input className={inputClass} value={filter.requestId} onChange={(event) => setFilter({ ...filter, requestId: event.target.value })} /></Field>
      </div>
          ...
          <table className="w-full min-w-[900px] text-left">
            <thead className="border-b border-border bg-muted/50 text-[11px] uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-2">Fresh In</th>
                <th className="px-4 py-2">Out</th>
                <th className="px-4 py-2">Cache Read</th>
                <th className="px-4 py-2">Cache Write</th>
                <th className="px-4 py-2">Reasoning</th>
                <th className="px-4 py-2">Real Total</th>
            ...
            <tbody className="divide-y divide-border">
              {result.content.map((record) => (
                <tr key={record.id} className="text-xs">
                  ...
                  <td className="px-4 py-2">{record.publicModel}</td>
                  <td className="px-4 py-2">{record.upstreamModel}</td>
                  <td className="px-4 py-2">{record.provider}</td>
                  <td className="px-4 py-2">{record.accountName ?? '—'}</td>
                  <td className="px-4 py-2">{record.apiKeyName ?? '—'}</td>
                  <td className="px-4 py-2">{freshInputTokens(record)}</td>
                  <td className="px-4 py-2">{record.outputTokens}</td>
                  <td className="px-4 py-2">{record.cacheReadTokens}</td>
                  <td className="px-4 py-2">{record.cacheCreationTokens}</td>
                  <td className="px-4 py-2">{record.reasoningTokens ?? '—'}</td>
                  <td className="px-4 py-2 font-medium">{record.realTotalTokens}</td>
                  <td className="px-4 py-2">{record.latencyMs} ms</td>
                  <td className="px-4 py-2">
                    {record.dataSource === 'ROUTER_CACHE' ? 'router' : record.isStreaming ? 'stream' : 'upstream'}
                  </td>
                </tr>
              ))}
            </tbody>
